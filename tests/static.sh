...
if grep -R -q -E 'xoxb-[A-Za-z0-9-]{10,}|xapp-[A-Za-z0-9-]{10,}|github_pat_' \
  "$ROOT" --exclude='static.sh' --exclude='openab.env.example' --exclude-dir='.git'; then
  printf 'A credential-like value is present in the repo.\n' >&2
  exit 1
fi

docker compose -f "$ROOT/compose.yaml" config --quiet
if grep -R -q -E 'nettop|Nettop|(^|[^[:alnum:]_])henry([^[:alnum:]_]|$)|(^|[^[:alnum:]_])Henry([^[:alnum:]_]|$)' \
  "$ROOT" --exclude='static.sh' --exclude='repos.conf' --exclude-dir='.git'; then
